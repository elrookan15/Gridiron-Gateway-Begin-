  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl">
      <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-800">
        ...
        <span className="px-2.5 py-1 rounded-full bg-lime-500/10 text-lime-400 text-xs font-mono font-bold border border-lime-500/20">
          5 Verified Events
        </span>
      </div>
      ...
      <div className="relative pl-6 border-l-2 border-slate-800 space-y-6">
        {MOCK_TIMELINE_EVENTS.map((event) => (
          <div key={event.id} className="relative group">
            {/* Timeline Dot */}
            <div className="absolute -left-[31px] top-1 w-6 h-6 rounded-full bg-slate-950 border-2 border-lime-500/50 flex items-center justify-center group-hover:scale-110 group-hover:border-lime-400 transition-all shadow-md">
            ...
            <div className="p-4 rounded-xl bg-slate-950/60 border border-slate-800/80 hover:border-slate-700 transition-all">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="px-2 py-0.5 rounded bg-slate-900 text-lime-400 text-[10px] font-bold border border-lime-500/30">
                    {event.badgeText}
                  </span>
                </div>
